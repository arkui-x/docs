...
- [FontDescriptor](#fontdescriptor)：字体描述符信息。
...
| 参数名 | 类型               | 必填 | 说明                              |
| ----- | ------------------ | ---- | --------------------------------------------------------------------------------- |
| action | [TextHighContrast](#texthighcontrast)  | 是   | 文字渲染高对比度模式。                                                              |
...
| 参数名 | 类型               | 必填 | 说明                              |
| ----- | ------------------ | ---- | ------------------------------------------------------------------------------- |
| noGlyphShow | [TextUndefinedGlyphDisplay](#textundefinedglyphdisplay) | 是   | 无法塑形字符的显示方式。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| - | - | - | - |
| desc | [FontDescriptor](#fontdescriptor) | 是 | 指定需要用来做匹配的字体描述符。如果不指定任何字段，则返回系统的所有字体描述符。如果填写了指定字段，则按照指定字段进行匹配。如果匹配失败，返回空数组。 |

**返回值：**

| 类型 | 说明 |
| - | - |
| Promise&lt;Array&lt;[FontDescriptor](#fontdescriptor)&gt;&gt; | Promise对象，返回所有匹配到的系统字体描述符。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| - | - | - | - |
| fontType | [SystemFontType](#systemfonttype) | 是 | 指定的字体类型。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| - | - | - | - |
| fullName | string | 是 | 指定的字体名称。可以使用[getSystemFontFullNamesByType](#textgetsystemfontfullnamesbytype)获取。 |
| fontType | [SystemFontType](#systemfonttype) | 是 | 指定的字体类型。 |
...
| 类型 | 说明 |
| - | - |
| Promise&lt;[FontDescriptor](#fontdescriptor)&gt; | Promise对象，返回指定的字体描述符。 |
...
| ---------------------------------- | ---- | ---------------------------------------------- | ---- | ---- |
| TEXT_FOLLOW_SYSTEM_HIGH_CONTRAST<sup>26+</sup> | 0 | 跟随系统设置中的高对比度文字配置。 | 支持 | 支持 |
...
| START     | 4    | 基于文本的方向[TextDirection](#textdirection)，文本靠开头方向对齐。 | 支持 | 支持 |
| END       | 5    | 基于文本的方向[TextDirection](#textdirection)，文本以结束方向对齐。 | 支持 | 支持 |
...
| BREAK_HYPHEN  | 3    | 每行末尾单词尝试通过连字符“-”进行断行，若无法添加连字符“-”，则跟`BREAK_WORD`保持一致。<br/>使用此断词策略时，需与[TextStyle](#textstyle)中`locale`属性配合使用，通过locale定义语言环境共同作用影响断词效果。                        | 支持 | 支持 |
...
| ------------------------- | --------------------------------------------------- | ---- | ---- | -------------------------------------------- | ---- | ---- |
| textDecoration            | [TextDecorationType](#textdecorationtype)           | 否   | 是   | 装饰线类型，默认为NONE。                       | 支持 | 支持 |
| color                     | [common2D.Color](js-apis-graphics-common2D.md#color)| 否   | 是   | 装饰线颜色，默认为跟随文本颜色。                       | 支持 | 支持 |
| decorationStyle           | [TextDecorationStyle](#textdecorationstyle)         | 否   | 是   | 装饰线样式，默认为SOLID。                      | 支持 | 支持 |
...
| 名称   | 值 | 说明      | Android平台 | iOS平台     |
| ------ | - | --------- | ---- | ---- |
| START  | 0 | 开头省略号，只在[ParagraphStyle](#paragraphstyle)中设置maxLines为1时生效。| 支持 | 支持 |
| MIDDLE | 1 | 中间省略号，只在[ParagraphStyle](#paragraphstyle)中设置maxLines为1时生效。| 支持 | 支持 |
| END    | 2 | 末尾省略号。| 支持 | 支持 |
...
| 名称          | 类型                                                 | 只读 | 可选 | 说明                               | Android平台 | iOS平台     |
| ------------- | ---------------------------------------------------- | --  | ---  | --------------------------------- | ---- | ---- |
| color         | [common2D.Color](js-apis-graphics-common2D.md#color) | 否  |  是   | 字体阴影的颜色，默认为黑色Color(255, 0, 0, 0)。        | 支持 | 支持 |
| point         | [common2D.Point](js-apis-graphics-common2D.md#point20) | 否  |  是   | 字体阴影基于当前文本的偏移位置，横、纵坐标要大于等于零。    | 支持 | 支持 |
...
| 名称               | 类型                                                 | 只读 | 可选 | 说明                                      | Android平台 | iOS平台     |
| -----------------  | ---------------------------------------------------- | --  | ---  | ---------------------------------------- | ---- | ---- |
| color              | [common2D.Color](js-apis-graphics-common2D.md#color) | 否  |  否   | 矩形框的颜色。                 | 支持 | 支持 |
...
| 名称                      | 类型                                     | 只读 | 可选 | 说明                                                   | Android平台 | iOS平台     |
| ------------- | ---------------------------------------------------- | -- | -- | --------------------------------------------------------- | ---- | ---- |
| decoration    | [Decoration](#decoration)                            | 否 | 是 | 装饰线设置，默认不使用装饰线。             | 支持 | 支持 |
| color         | [common2D.Color](js-apis-graphics-common2D.md#color) | 否 | 是 | 文字颜色，默认为白色。                         | 支持 | 支持 |
| fontWeight    | [FontWeight](#fontweight)                            | 否 | 是 | 字重，默认为W400。 目前只有系统默认字体支持字重的调节，其他字体设置字重值小于semi-bold（即W600）时字体粗细无变化，当设置字重值大于等于semi-bold（即W600）时可能会触发伪加粗效果。                         | 支持 | 支持 |
| fontStyle     | [FontStyle](#fontstyle)                              | 否 | 是 | 字体样式，默认为常规样式。                          | 支持 | 支持 |
| baseline      | [TextBaseline](#textbaseline)                        | 否 | 是 | 文本基线类型，默认为ALPHABETIC。               | 支持 | 支持 |
...
| ellipsisMode  | [EllipsisMode](#ellipsismode)                        | 否 | 是 | 省略号类型，默认为END，行尾省略号。                       | 支持 | 支持 |
...
| fontFeatures  | Array\<[FontFeature](#fontfeature)>                  | 否 | 是 | 文本字体特征数组。| 支持 | 支持 |
| fontVariations| Array\<[FontVariation](#fontvariation)>              | 否 | 是 | 可变字体属性数组。| 支持 | 支持 |
| textShadows   | Array\<[TextShadow](#textshadow)>                    | 否 | 是 | 文本阴影数组。| 支持 | 支持 |
| backgroundRect| [RectStyle](#rectstyle)                              | 否 | 是 | 文本矩形框样式。| 支持 | 支持 |
| badgeType   | [TextBadgeType](#textbadgetype) | 否   | 是   | 设置文本排版时是否使能上标或下标。TEXT_SUPERSCRIPT表示使能上标，TEXT_SUBSCRIPT表示使能下标，默认值为TEXT_BADGE_NONE表示不使能。| 支持 | 支持 |
...
| fontFamilies   | Array\<string>                                       | 否   | 是 | 字体家族名称列表，默认为空，匹配系统字体。                                               | 支持 | 支持 |
| fontStyle      | [FontStyle](#fontstyle)                              | 否   | 是 | 字体样式，默认为常规样式。                                               | 支持 | 支持 |
| fontWidth      | [FontWidth](#fontwidth)                              | 否   | 是 | 字体宽度，默认为NORMAL。                                                | 支持 | 支持 |
| fontWeight     | [FontWeight](#fontweight)                            | 否   | 是 | 字重，默认为W400。系统默认字体支持字重调节，其他字体设置字重值小于W600时无变化，大于等于W600时可能触发伪加粗效果。                             | 支持 | 支持 |
...
| weight | [FontWeight](#fontweight) | 否 | 是 | 字体字重，默认值为0。 | 支持 | 支持 |
...
| path  | string \| [Resource](../arkui-ts/ts-types.md#resource) | 是   | 需要导入的字体文件的路径，应为 "file:// + 字体文件绝对路径" 或 "rawfile/目录or文件名"。 |
...
|   path   | string \| [Resource](../arkui-ts/ts-types.md#resource) | 是   | 需要加载的字体文件的路径，支持两种格式： "file:// + 字体文件绝对路径" 或 "rawfile/目录or文件名"。 |
...
| 名称                 | 类型                                        | 只读 | 可选 | 说明                                          | Android平台 | iOS平台     |
| -------------------- | ------------------------------------------ | ---- | ---- | -------------------------------------------- | ---- | ---- |
| textStyle            | [TextStyle](#textstyle)                    | 否   | 是   | 作用于整个段落的文本样式，默认为初始的文本样式。| 支持 | 支持 |
| textDirection        | [TextDirection](#textdirection)            | 否   | 是   | 文本方向，默认为LTR。                         | 支持 | 支持 |
| align                | [TextAlign](#textalign)                    | 否   | 是   | 文本对齐方式，默认为START。若同时配置tab属性，制表符对齐方式将失效。 | 支持 | 支持 |
| wordBreak            | [WordBreak](#wordbreak)                    | 否   | 是   | 断词类型，默认为BREAK_WORD。                    | 支持 | 支持 |
| maxLines             | number                                     | 否   | 是   | 最大行数限制，整数，默认为1e9。                  | 支持 | 支持 |
| breakStrategy        | [BreakStrategy](#breakstrategy)            | 否   | 是   | 断行策略，默认为GREEDY。                        | 支持 | 支持 |
| strutStyle           | [StrutStyle](#strutstyle)                  | 否   | 是   | 支柱样式，默认为初始的StrutStyle。               | 支持 | 支持 |
| textHeightBehavior   | [TextHeightBehavior](#textheightbehavior)  | 否   | 是   | 文本高度修饰符模式，默认为ALL。                              | 支持 | 支持 |
| tab   | [TextTab](#texttab)  | 否   | 是   | 表示段落中文本制表符后的文本对齐方式及位置，默认将制表符替换为一个空格。此参数与文本对齐方式（align属性）或省略号样式（[TextStyle](#textstyle)中的ellipsis属性）共同配置时无效。 | 支持 | 支持 |
...
| verticalAlign   | [TextVerticalAlign](#textverticalalign) | 否   | 是   | 文本垂直对齐方式，开启行高缩放（即设置[TextStyle](#textstyle)的heightScale）或行内不同字号（即设置[TextStyle](#textstyle)的fontSize）文本混排时生效。若行内有上下标文本（即设置[TextStyle](#textstyle)的badgeType属性文本），上下标文本将与普通文本一样参与垂直对齐。 | 支持 | 支持 |
...
| align          | [PlaceholderAlignment](#placeholderalignment) | 否   | 否   | 相对于周围文本的纵向对齐方式。| 支持 | 支持 |
| baseline       | [TextBaseline](#textbaseline)                 | 否   | 否   | 基线类型。                   | 支持 | 支持 |
...
| count | number | 是   | 从指定起始位置开始进行排版的字符个数，取值为[0,文本字符总数)的整数，startIndex和count之和不能大于文本字符总数。当count为0时，表示排版区间为[startIndex, 文本结尾]。可以先使用[getLineBreak](#getlinebreak)获取合理的排版字符总数。|
...
| ----------- | ------------------------------------------------------------ | ---- | ---- | ---------------- | ----------- | ------- |
| textStyle   | [TextStyle](#textstyle)                                      | 否   | 否   | 字体的样式信息。 | 支持        | 支持    |
| fontMetrics | [drawing.FontMetrics](arkts-apis-graphics-drawing-i.md#fontmetrics) | 否   | 否   | 字体度量信息。   | 支持        | 支持    |
...
| runMetrics   | Map<number, [RunMetrics](#runmetrics)>                        | 否   | 否   | 文本索引范围与关联的字体度量信息之间的映射。| 支持 | 支持 |
...
| 名称      | 类型                                                | 只读 | 可选 | 说明        | Andriod平台 | iOS平台 |
| --------- | -------------------------------------------------- | ---- | ---- | ----------- |----------- |----------- |
| rect      | [common2D.Rect](js-apis-graphics-common2D.md#rect) | 否   | 否   | 矩形区域信息。| 支持 | 支持 |
| direction | [TextDirection](#textdirection)                    | 否   | 否   | 文本方向。    | 支持 | 支持 |
...
| affinity  | [Affinity](#affinity) | 否   | 否   | 位置亲和度。               | 支持 | 支持 |
...
| 类型                     | 说明                           |
| ------------------------ | ------------------------------ |
| [LineTypeset](#linetypeset)  | 可用于后续渲染的LineTypeset对象。|
...
| 类型 | 说明  |
| -| - |
| [TypographicBounds](#typographicbounds) | 文本行的排版边界。|
...
| 参数名 | 类型 | 必填 | 说明 |
| -| - | - | - |
| point | [common2D.Point](js-apis-graphics-common2D.md#point20) | 是 | 要查找索引的位置。|
...
| 参数名 | 类型 | 必填 | 说明 |
| -| - | - | - |
| callback | [CaretOffsetsCallback](#caretoffsetscallback) | 是 | 用户自定义函数。回调方法参数包括文本行中每个字符的偏移量和索引值。 |
...
| 类型                   | 说明                                   |
| ---------------------- | ------------------------------------- |
| Array\<[common2D.Point](js-apis-graphics-common2D.md#point20)> | 该排版单元中每个字形相对于每行的字形位置。 |
...
| 类型                   | 说明           |
| ---------------------- | -------------- |
| Array\<[common2D.Point](js-apis-graphics-common2D.md#point20)> | 该排版单元中每个字形相对于其索引的偏移量。|
...
| 类型                   | 说明           |
| ---------------------- | -------------- |
|  [TypographicBounds](#typographicbounds)  | 该排版单元的排版边界。|
...
| 类型                   | 说明                                   |
| ---------------------- | ------------------------------------- |
| Array\<[common2D.Point](js-apis-graphics-common2D.md#point20)> | 返回该排版单元中每个字形相对于水平方向的字形宽度数组。其中，[common2D.Point](js-apis-graphics-common2D.md#point20)中的x值代表每个字形相对于水平方向的字形宽度，y值为保留字段，默认返回0。 |
...
| 名称               | 类型                    | 只读 | 可选 | 说明                                               | Andriod平台 | iOS平台 |
| -----------------  | ----------------------- | ---- | ---  | -------------------------------------------------- |----------- |----------- |
| alignment          | [TextAlign](#textalign) | 否   |  否  | 段落中制表符之后的文本对齐方式，支持设置[TextAlign](#textalign)的LEFT左对齐、RIGHT右对齐和CENTER居中对齐方式，其他枚举值为左对齐，默认为左对齐。 | 支持 | 支持 |
