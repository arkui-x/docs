...
请检查WebviewController对象是否已与Web组件关联，可以通过[onControllerAttached()](../arkui-ts\ts-basic-components-web.md#oncontrollerattached10)接口进行检查。
...
使用的错误码不在[WebNetErrorList](../apis/arkts-apis-netErrorList.md#webneterrorlist)范围内，或者调用didFail接口时使用了NET_OK错误码。

**处理步骤**

检查使用的错误码是否在[WebNetErrorList](../apis/arkts-apis-netErrorList.md#webneterrorlist)范围内，或者调用difFail时是否使用了NET_OK错误码。
