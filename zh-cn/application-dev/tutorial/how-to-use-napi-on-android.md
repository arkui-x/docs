...
在OpenHarmony中，N-API接口可以实现ArkTS/TS/JS与C/C++(Native)之间的交互。ArkUI-X在此基础上进行了跨平台扩展，N-API接口可以在OpenHarmony/HarmonyOS/Android/iOS多个平台上使用，N-API提供的接口名与三方Node.js一致，目前支持部分接口，支持列表见[链接](../reference/native-lib/third_party_napi/napi.md)。本文档以[ACE Tools工具](../quick-start/start-overview.md)创建的Native样例工程为例，介绍如何在Android平台上，使用N-API能力完成跨语言调用。

## 开发流程

使用N-API完成跨语言调用的整体开发流程分为以下五步，即
1. 环境准备：创建Native工程。
2. 提供Native能力：使用N-API接口提供Native能力，同时，Native接口中还可调用ArkTS能力。
3. 定义接口声明：添加Native侧暴露给ArkTS侧接口的声明。
4. 调用Native能力：ArkTS/TS/JS侧使用Native能力。
5. 编译与运行应用：利用[ACE Tools工具](../quick-start/start-overview.md)编出apk应用，并在android平台上运行，观察效果。

这里以样例工程的两个例子来介绍N-API的典型使用场景：
...
```C++
// source\entry\src\main\cpp\hello.cpp，包含Native侧逻辑。
// 引入N-API相关头文件。
...
在`d.ts`文件中添加暴露接口声明
```js
// source\entry\src\main\cpp\types\libentry\index.d.ts，包含Native侧暴露给ArkTS侧接口的声明。
...
```js
// source\entry\src\main\ets\pages\Index.ets，包含ArkTS侧逻辑。
...
### 5. 编译与运行应用
利用[ACE Tools工具](../quick-start/start-overview.md)编译出apk包，并在Android平台上运行。
```
ace run apk
```
应用运行后，点击页面上的两个文本，文本做出相应变换。
![运行效果](napi-demo.png)
