...
**Q1：**上面讲到的都是开源API不支持跨平台时的处理策略，对于均不支持跨平台的HMS API，直接基于上述写法运行起来会直接crash，以活体检测API：`import interactiveLiveness from '@hms.ai.interactiveLiveness'`​为例，运行态会报如下截图错误，该如何解决？
