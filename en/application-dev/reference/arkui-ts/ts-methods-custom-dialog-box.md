...
| gridCount<sup>8+</sup>        | number                                   | No   | Number of [grid columns](ts-container-gridrow.md) occupied by the dialog box.<br>The default value is subject to the window size, and the maximum value is the maximum number of columns supported by the system. If this parameter is set to an invalid value, the default value is used. |
